...
surfacewater, unsound practices
...
well and levellogger 1065861 was deployed in a sheltered open air area. Both
loggers recorded synchronous 15-minute data, further details are available in
@vangrinsven-2017. In the described deployment scenarios from above this would
...
evapotranspiration and groundwater recharge was first proposed by @white-1932.
...
expanded to ecosystem specific yield to acknowledge the influence of factors
beyond the substrate surrounding the well [@mclaughlin-2014]. We applied the
adjusted White methodology to the 15-minute wetland well data to calculate daily
evapotranspiration for the corrected and uncorrected water level records. Our
estimates of ecosystem specific yield were derived as the ratio between the
change in water level and the change in water availability (precipitation plus
snowmelt less potential evapotranspiration). This is the inverse analog of rain
to rise ratio used in other studies [@mclaughlin-2014]. We used this analogous
approach because our site is influenced by two factors known to obfuscate a
clear ecosystem specific yield relationship: surface water exchange
[@watras-2017] and additional hydrologic cycles at longer wavelengths (e.g.
seasonal inundation patterns) [@zhu-2011]. For validation of the correction
procedure We compared the corrected and uncorrected estimates to potential
evapotranspiration (PET) calculated using the modified Hargreaves-Samani
equation @hargreaves-2003 with data from nearby meteorological stations
(Shannon, in preparation).
...
daily_wl_errors_percent <- 
  paste(round(daily_wl_errors_n_days / daily_wl_comparisons, 1), '%')

```


When mean raw and corrected water levels are compared `r daily_wl_errors_n_days`
days (`r daily_wl_errors_percent`) were found to have temperature corrections
that exceeded combined instrument error (Figure \@ref(fig:case-study-panel)).
Daily mean errors were greatest in the fall, and smallest during the summer
months, ranging up to a 4 cm overestimation of mean daily water level.

Need to evaluate Esy equation for corrected and uncorrected (visually they look
about the same)

The days highlighted in the panel (with likely flawed estimates
of ET) show a mean difference less than instrument error

```{r case-study-panel, fig.width=7.5, fig.cap='A comparison of corrected and uncorrected wetland water levels. Panel A shows the entire growing season at 15-minute interval. Panel B shows the error in daily mean water level. The vertical shaded bar in Panels A and B highlight the two days shown in Panel C. Panel C shows details of two diurnal water level cycles and the decomposed components of the @white-1932 method.'}
...
Larger errors in the fall can be explained by seasonal temperature patterns. In
spring air is cold and water is cold from snowmelt. In fall air is cold, but 
water is warmer due to rain & groundwater as sources, rather than recent 
snowmelt.

There are not enough calibration measurements to truly analyze which daily mean
is 'correct', but based on the experiment in this study we are working from the
assumption that the temperature-corrected values are. The second analysis is
easier to evaluate because we can compare the results to the relationship with 
PET. 

...
well at the same depth) (@mclaughlin-2011). However, this type of deployment is not
...
'temperature-difference-biased' (transducers deployed without regard to similar
...
practice' is enough to eliminate temperature-induced errors.
...
temperature differential is likely the response of individual transducers to of
...
```{r drivers-panel, fig.cap="Error is driven by a combination of individual transducer errors, which can result in masking by the stronger trend. Panel A shows water level error as a function of air temperature, illustrating a strong linear trend. Panel B shows a dichotomous relationship between water level error and water temperature. Panel C is the relationship between error and water temperature after the influence of air temperature is removed via OLS linear regression. Data taken from a barometric pressure transducer 1066019 and water pressure transducer 1062452 during the SW~bias-2~ period."}

knitr::include_graphics("../output/figures/sources_of_error_scatter_plot_panel.tiff")
...
logging. These transducers also provide internal temperature compensation of
pressure measurements.

```{r experiments, results = 'asis'}
experiments <-
  data.table(Scenario = c("GW~best~", "GW~bias~", "SW~best~", "SW~bias-1~", "SW~bias-2~"),
             ...
             `Air/Water Temperature Regime` = c("Similar", "Dissimilar", "Similar", "Dissimilar", "Dissimilar"))

flextable(experiments) %>%
  # colformat_md() %>%
  set_caption("Description of experimental conditions. Best practice describes air and water pressure transducers deployed under similar thermal regimes.",
...
with the transducers zero points aligned (\@ref(fig:apparatus)). The transducers
were covered with a fixed depth of water and a layer of canola oil was added to
prevent evaporation over the length of the experiment. Water level was measured
at the start and end of the experimental period to confirm no evaporation. To
create the best- practice condition of thermally similar air and water
environments we placed a weighted graduated cylinder in the center of the
container. During the two periods with synchronous thermal regimes the two
barometric transducers were suspended in the cylinder below the water surface.
The entire apparatus was covered loosely with aluminum foil to prevent direct
wind effects.

```{r apparatus, fig.cap="Diagram showing arrangement of pressure transducers within experimental apparatus. During the study two barometric transducers were deployed in the central tube in best-practice study scenarios. During all study periods 18 water transducers were susspended from two cross braces."}
knitr::include_graphics("../output/figures/experimental_apparatus.png")
...
knitr::include_graphics("../output/figures/bootstrap_correction_timeseries_linegraph_and_ribbon_panel.tiff")
```
...
denotes the corrected water level. For analysis and plotting constant transducer
...
the bootstrapped models derived from the SW~bias-2~ period were evaluated
...
season of 2018. Transducer 1066016 was deployed in a 5.08cm I.D. monitoring
well and transducer 1065861 was deployed in a sheltered open air area with both
transducers recorded synchronous 15-minute data [@vangrinsven-2017]. In the
...
```{r prediction-rmse}
rmse_values <- 
  melt(predicted[, c(instrument_error_cm = qnorm(0.975) * instrument_error_cm[1], 
                         lapply(.SD, function(x) sqrt(mean((x - water_depth_cm)^2, na.rm = TRUE)))), 
                   by = .(water_sn, baro_sn, experiment),
                   .SDcols = c("centered_water_level_cm", "rect_water_level_cm")], 
       id.vars = c("water_sn", "baro_sn", "experiment", "instrument_error_cm"),
...
# # Is RMSE less than instrument error?
# rmse_values[type == "rect_water_level_cm",
#             t.test(rmse_cm, instrument_error_cm, data = .SD,
#                    alternative = "less")[c("statistic", "p.value")],
#             by = .(baro_sn, experiment)]
...
0.89 to 0.98. The bootstrap predictions of final WL~corr~ had RMSE values that
ranged from 0.3208 to 0.5092 cm, which includes the uncertainty inherent in the
instrument measurements. For reference observed error for that period ranged
from -15.98 to 10.80 cm. When the SW~bias-2~ model was used to predict other
experimental periods, the RMSE of raw and corrected water level errors relative
to true water level showed a mean change of -0.001, -0.058, 0.042, and -0.614 cm
within the GW~best~, GW~bias~, SW~best~, and SW~bias-1~ experimental periods
respectively, where a negative value indicates a smaller RMSE following
correction. The reduction in RMSE for the GW~bias~ and SW~bias-1~ period were
significant at the $\alpha=0.05$ level, as was the increase in RMSE in the
SW~best~ period. When the two models that that showed outlier model performance
(water transducers 2025928 and 2030899, Figure \@ref(fig:coefficients)) were
removed, the increase in RMSE in the SW~best~ period was reduced to -0.018 and
was no longer significant. When the RMSE of WL~corr~ was compared to instrument
error bounds using a t-test with the $\alpha=0.05$ and $H_A=\hat{\epsilon}_{L} -
\epsilon_{instrument} < 0$, we found RMSE of WL~corr~ was significanlty less
than instrument error for all experimental periods.

```{r coefficients, fig.cap="Mean and 95% confidence interval for model coefficients for the 1000 bootstrap models for the SW~bias-2~ period. Panels A and C show coefficients grouping by barometric transducer and panel B shows models grouping in pairs for each water-barometric transducer pair. Water transducers 2025928 (hollow circle) and 2030899 (hollow inverted triangle) are highlighted to illustrate disagreement with other models for one or more coefficients.", fig.width=6, fig.height=4}

knitr::include_graphics("../output/figures/bootstrap_coefficients_pointrange_panel.tiff")
...
predicted[, `:=`(raw_ooir = centered_water_level_cm < instrument_lower | centered_water_level_cm > instrument_upper, 
                 rect_ooir = rect_water_level_cm < instrument_lower | rect_water_level_cm > instrument_upper,
                 raw_oopr = centered_water_level_cm < propagated_lower | centered_water_level_cm > propagated_upper, 
...
flextable(oor_percent_all_points) %>% 
    delete_part('header') %>% 
    add_header_row(values = c("", "WL~raw~", "WL~corr~", "WL~raw~", "WL~corr~")) %>% 
    add_header_row(values = c("Experiment", "Instrument Error", "Propagated Error"),
                   colwidths = c(1, 2, 2)) %>% 
    hline_top(border = fp_border(width = 1.5)) %>% 
    fit_to_width(max_width = 8) %>% 
    align(align = "center", part = 'all') %>% 
  set_caption("Percentage of observations of raw and corrected water levels falling outside of instrument accuracy range and propagated error range.",
              autonum = run_autonum(seq_id = "tab", bkm = "oor"))
```
...
flextable(unc_table) %>% 
    # colformat_md() %>% 
    set_caption("Mean Propagated and Instrument for all 36 transducer pair models by experimental period. Difference and standard error shows the change in confidence band size when all errors are incorporated and were tested for significance using a paired t-test within each experimental period (denoted by '*'). Difference in propagated errors between experimental periods was tested using a one-way ANOVA.",
                autonum = run_autonum(seq_id = "tab", bkm = "uncertainty")) %>% 
...
among experimental treatments from `r unctests[["GW~best~"]]`cm for GW~best~ 
to `r unctests[["SW~bias-2~"]]`cm for SW~bias-2~ (Table \@ref(tab:uncertainty)).
...
```{r new-errors}
vardis <- predicted[experiment == "SW~bias-1~"]
...
# write.csv(vardis_errors, row.names = FALSE, "output/writing/Table_S2_transducer_pair_mean_propagated_errors.csv")
...
knitr::include_graphics("../output/figures/case_study_panel.tiff")
```
...
ET~raw~ showed negligible correlation with PET (`r pet_cor["raw_et_cm_d"]`), while 
ET~corr~ showed moderate correlation with PET (`r pet_cor["corrected_et_cm_d"]`). 
...
knitr::include_graphics("../output/figures/et_to_pet_panel.tiff")
...
The uncertainty of a final water depth measurement is composed of the error of
each involved instrument and all necessary compensation models. Therefore by 
removing any dependency on external instrumentation we minimize the total 
uncertainty of the corrected water level measurement. 

Importantly, the presented approach faithfully captures the individual
transducer corrections. One possibility of the two-transducer model is that the
model would be fit to some artifact of the interaction between the transducers,
rather than each transducer being represented in the air and water temperature
coefficients. Figure \@ref(fig:coefficients) shows that the air temperature
coefficients were grouped by barometric transducer (two distinct regions of
overlapping ranges), and the water temperature coefficients were grouped by
water transducer (18 distinct regions of overlap of the two models representing
each water transducer). Clear transducer differentiation also supports the
hypothesis that water level errors using absolute pressure transducers are
introduced through independent water and air temperature errors not temperature
differences. Correcting for water level error using temperature difference would
disregard the differential transducer responses. Based on the scale of the
coefficients for air temperature and water temperature, these errors would
likely be small in this study. But the scale of the error would be directly
proportional to the sensitivity of transducer to temperature and would vary
between manufacturers and instruments.
...
suitable for varied deployment environments. However, we recommend developing
compensation equation under conditions similar to field conditions whenever
possible to ensure accurate compensation. The degraded predictive performance of
SW~bias-2~ models under SW~best~ conditions relative to all other periods
highlights this point. Under SW~best~ conditions we observed a small but
significant increase in RMSE for corrected water levels. We believe that the
inaccuracies have two potential causes at their root. The first is that the
SW~best~ conditions had higher temperatures than any of the other experimental
periods. Models developed on SW~bias-2~ data were therefore extrapolating when
predicting under SW~best~ conditions. The second possible reason for decreased
predictive power is the internal temperature compensation of the pressure
transducers. @freeman-2004 anticipated that isolating and correcting
temperature-driven errors would be more difficult in transducers with internal
compensation. The data used to train our models were purposefully collected
under the extreme conditions (SW~bias-2~), with large, rapid temperature
fluctuations in both the air and water readings. Under these conditions the
internal temperature compensation algorithm is likely overwhelmed. Under
conditions with smaller, slower temperature fluctuations (SW~best~), the
compensation models may overcorrect the data because the internal temperature
compensation has already dampened the temperature-induced errors. Identifying
extrapolation/internal-compensation driven errors illustrates the importance of
using an independent dataset to validate compensation models. Use of multiple
independent validation sets also enables testing a single compensation model for
use in situations where monitoring conditions may vary significantly throughout
transducer deployment. Using a single model across varied monitoring conditions
comes with the caveat of ensuring that the total range of conditions have been
...
temperature and high rates of temperature change than transducer 1066019. This
led to larger errors at the beginning and end of the SW~bias-1~ period (Figure
\@ref(fig:bootstrap-timeseries)). This is further supported by the increased
error associated with the compensation models for transducer 1065861 relative to
transducer 1066019 (**SUPPLEMENTAL TABLE 1**). Transducers 2025928 and 2030899
also show larger errors, and account for all of the increase in OOR measurements
in the SW~best~ period. When looking at the distribution of model RMSEs,
transducer 2013939 showed no overlap in the RMSE range of all other models.
While this suggests that the transducer may be overfit to the training data, it
appears that during the SW~best~ period the direction of the relationship
between water temperature and error reversed relative to all other experimental
periods (\@ref(fig:coefficients)B). The same occurred for transducers 2064734 and 1062528. At this time it
is impossible to tell if this is due to the high temperatures during this
period, differential internal compensation among transducers, or some other
error such as a trapped air bubble against the transducer membrane. The
experimental and modeling approach laid out above can, and should, be used to
adjust the compensation equations for individual transducers such as 2025928,
2013939, and 1065861. That scale of tuning is outside of the scope of this
paper, which is instead focused on commonalities in compensation that can be
applied across instruments and manufacturers and the role of temperature-derived
errors in total uncertainty. The above discussion highlights an advantage of 
simultaneous compensation of multiple instruments. By including multiple pairs 
of barometric and water pressure transducers, we are able to identify and 
isolate additional errors to specific the barometric or water transducer. If 
multiple transducers are not available for simultaneous compensation performing 
preliminary or post-correction checks against an external data source is 
recommended.
...
<!-- compensation approaches across multiple transducers. -->

```{r sigma-plots, eval = FALSE}
{ggplot(bootstrap_models[experiment == "SW~bias-2~"],
        aes(x = air_temperature_c,
            fill = baro_sn, group = model_id)) +
        geom_density(alpha = 0.5) + ggtitle("SW~bias-2~")} / {ggplot(bootstrap_models[experiment == "SW~bias-1~"],
       aes(x = air_temperature_c,
           fill = baro_sn, group = model_id)) +
    geom_density(alpha = 0.5) + ggtitle("SW~bias-1~")}

ggplot(bootstrap_models[experiment == "SW~bias-2~"],
        aes(x = water_temperature_c, y = ..scaled..,
            fill = baro_sn, group = model_id)) +
        geom_density(alpha = 0.5) + ggtitle("SW~bias-2~") + facet_wrap(~water_sn, scales = "free")
...
errors. We found that when deploying transducers to variable temperature
...
correlated with environmental conditions. This correlation is especially vexing
because the points that are most likely to lie outside of the error bounds occur
simultaneously with periods of hydrologic interest. As the errors are associated
with higher temperatures and high rates of temperature change, the errors will
often co-occur with changes in flow following snowmelt or rain events or periods
of peak evapotranspiration. To confidently attribute diurnal or seasonal signals
to true variation rather than error requires additional validation. One source
of validation is additional instrumentation not susceptible to
the same error source or frequent manual measurements as in [@cuevas-2010;
@gribovszki-2013]. Where this is not practical or possible, we have shown that
compensation equations can quantify the additional uncertainty of 
...
i_mods <- 
  bootstrap_models[, 
                   .(experiment, 
                     N = as.numeric(pmax(abs(air_temperature_c), 
                                         abs(water_temperature_c), 
                                         abs(delta_at_01c_min)) == 0), sigma)
                   ][, .(mean = mean(N),
                         se = sqrt(var(N)/.N),
                         sigma = median(.SD[N > 0, sigma])), 
                     keyby = .(experiment)]
...
reduce compensation errors. Even under 'best practices' thermal regimes errors
will occur for many transducers, though the magnitude of the error may be
similar to instrument error and may not be apparent. Our results show that even
if not apparent, this error should be considered because it represents
systematic bias rather than a random error. Of the 1000 models fit per
experimental period, `r imods[["GW~best~"]]`% of GW~best~, 
`r imods[["GW~bias~"]]`% of GW~bias~, and `r imods[["SW~best~"]]`% of SW~best~ 
final models were intercept-only. Under increasingly ideal conditions more
models showed no significant relationship with air temperature, water
temperature, or rate of change of air temperature. The error associated with
these intercept only models still provides important information regarding true
water level accuracy and they are included in the propagated error estimates. As
intercept only, these models are likely not devoid of temperature derived
errors, and in fact show slightly, but significantly higher $\sigma_{mod}$ than
other models ( `r sigma_string`). This small increase in $\sigma_{mod}$ is
likely the result of unmodeled temperature-driven errors. As temperature
artifacts were present under all conditions it is not unexpected that we found
that true uncertainty was greater than instrument error under all conditions.
Taken together it is likely that even under ideal conditions thermal artifacts
will be affecting measurement accuracy in a systematic way and compensation is
recommended.
...
accurate error estimates, corrected observations, which results in narrower
...
quantify the error of newly corrected observations and reduce final measurement
uncertainty. In our study our best performing compensation models showed a 65%
reduction in the uncertainty range, from 1.19 cm to 0.43 cm. A reduction of this
magnitude is not insignificant in the scale of diurnal signals often studied.
...
introduced due to temperature-induced errors in WL~raw~. The purpose of water
level records in monitoring and research is to enable further analysis.
Unaccounted for errors in instrument records have measurable impacts, which
could lead to erroneous conclusions and potentially to real-world actions that are
...
conditions led to the largest errors in our laboratory experiments. As the air
...
and low water temperatures due to snowmelt. Our observed daily mean error were 
of similar magnitude the 1.5 cm increase in diurnal variation observed by 
McLaughlin _et al_ [@mclaughlin-2011]. Our estimates of ET error were much 
larger than the 19% increase in streamflow diurnal fluctuation presented in 
Cuevas _et al_ [-@cuevas-2010], though ET is often a smaller component of the 
water budget so that larger percentage errors can be expected.
...
functions were similar for the raw and corrected datasets (**SUPPLEMENTAL
MATERIAL**). It is important to note that while ET~corr~ showed a moderate
correlation with PET and a significant slope near 1, the R^2^ of the OLS model
was low. This is in line with other White-method separation studies where low
R^2^ values are observed. @soylu-2012 found R^2^ values of less than 0.2 for the
White method and 0.33&ndash;0.4 for their proposed sine-wave based method.
@mclaughlin-2014 does not report R^2^, but their $ET/PET$ index shows high
variability, suggesting low R^2^. Even with direct eddy flux measurement of ET,
@lafleur-2005 found an R^2^ as low as 0.56 between ET and PET in a Canadian
...
## Uncertainty and Transducer Type

This study focused on absolute pressure transducers, but some of the findings
could inform similar work on gauge pressure transducers. @cain-2004 found that
temperature changes within the vent-tube of gauge pressure transducers introduce
water level artifacts, which was explored further by
...
change affecting the connection to the atmosphere via the vent tube. They also
highlight the impact of rate of temperature change, with more rapid gradients
inducing larger water level errors, which agrees with the results from our
study. Agreement between the two logger types is expected as both contain
internal electronics and membranes that must equilibrate to the new temperatures 
[@freeman-2004].
...
transducers the added uncertainty is not the result of temperature differentials,
but is a combination of individual transducer biases. These biases can impact
uncertainty across commonly used deployment scenarios for hydrological 
monitoring. Even under the best practices, individual logger response to similar
thermal regimes will result in increased uncertainty, and can result in 
measurable water level errors. Transducer and tranducer-pair specific 
compensation equations can be used to reduce errors and fully quantify 
measurement uncertainty. In addition laboratory-derived compensation can be used 
to reduced final uncertainty below stated instrument error, providing a 
mechanism for higher resolution analyses. We provide the below recommendations 
for addressing and correcting temperature errors.

- For each transducer check for *all* potential error-inducing responses to 
thermal gradients and rates of change in temperature. 

- Even under best practices thermal error should be considered. This is 
especially true if monitoring water sources that vary in temperature, e.g. 
surfacewater and shallow groundwater.

- Compensation equations should be developed under conditions that capture the 
full range of temperatures likely to be monitored.

- To maximize compensation benefits collect validation datasets separate from
data used to derive equations. These datasets can be used to create new and 

...
@kirchner-2009; @loheideii-2005; @mclaughlin-2014; @watras-2017; @white-1932].
...
relatively constant temperature and utilize a fixed water density of 1000 $kg m^{-3}$ to convert pressure to depth (Personal Communication, Solinst). To
...
standard deviation equal to the known instrument accuracy for each variable (Table
...
loggers recorded synchronous 15-minute data [@vangrinsven-2017]. In the described deployment scenarios from above this would
be unsound groundwater monitoring. We will illustrate the impact of temperature 
bias on two common analyses: mean daily water level and evapotranspiration 
derived from diurnal water level variation. For the former analysis we calculate
mean uncorrected and corrected water levels on a daily timestep and compare the
difference between them.  

An approach for decomposing daily groundwater level records into
evapotranspiration and groundwater recharge was first proposed by White [-@white-1932].
The approach involves estimating net inflow rate during a recharge period
typically the pre-dawn hours. This rate and the change in water level between
recharge periods can be used to determine evapotranspiration. Converting water
level fluctuation to surface evapotranspiration also requires accounting for the
specific yield of the substrate surrounding the well. Loheide [-@loheideii-2008;
-@loheideii-2005] expanded this work to improve estimates of specific yield and
account for variation in groundwater flow with water level change. The White and
Loheide works focused on groundwater-phreatophyte interactions, but the method
has been extended to use in wetland settings [@diamond-2018; @mclaughlin-2014;
@telander-2015]. In these setting Loheide's readily available specific yield was
expanded to ecosystem specific yield (E~Sy~) to acknowledge the influence of
factors beyond the substrate surrounding the well [@mclaughlin-2014]. We applied
the adjusted White methodology to the 15-minute wetland well data to calculate
daily evapotranspiration for the corrected and uncorrected water level records.
Our estimates of E~Sy~ were derived as the ratio between the change in water
level and the change in water availability (precipitation plus snowmelt less
potential evapotranspiration) for a previous year of data at the same location.
This is an inverse analog of rain to rise ratio used in other studies
[@mclaughlin-2014]. We used this analogous approach because our site is
influenced by two factors known to obfuscate a clear E~Sy~ relationship: surface
water exchange [@watras-2017] and additional hydrologic cycles at longer
wavelengths (e.g. seasonal inundation patterns) [@zhu-2011]. For validation of
the correction procedure We compared the corrected and uncorrected estimates to
potential evapotranspiration (PET) calculated using the modified
Hargreaves-Samani equation @hargreaves-2003 with data from nearby meteorological
stations (Shannon, in preparation).
...
daily_wl_comparisons <- 
  water_balance[,.(raw_wl = mean(raw_compensated_level_cm, na.rm = TRUE), 
                   corrected_wl = mean(corrected_compensated_level_cm, na.rm = TRUE),
                   instrument_error_cm = median(instrument_error_cm, na.rm = TRUE)),
                by = .(sample_date)][, error_cm := raw_wl - corrected_wl]

daily_wl_errors_n_days <- 
  sum(daily_wl_comparisons$error_cm > daily_wl_comparisons$instrument_error_cm)

daily_wl_errors_percent <- 
  paste(round(daily_wl_errors_n_days / nrow(daily_wl_comparisons), 1), '%')

max_daily_error <- 
  round(abs(max(daily_wl_comparisons$error_cm, na.rm = TRUE)), 1)

min_daily_error <- 
  round(abs(min(daily_wl_comparisons$error_cm, na.rm = TRUE)), 1)
...
For much of the study period the overall trend in water levels track well
between the raw and corrected levels (Figure \@ref(fig:case-study-panel)A).
Differences become noticeable when the common analyses are tested. When mean raw
and corrected water levels are compared `r daily_wl_errors_n_days` days (
`r daily_wl_errors_percent`) were found to have temperature corrections that
exceeded combined instrument error (Figure \@ref(fig:case-study-panel)B). Daily
mean errors were greatest in the fall, and smallest during the summer months,
ranging from a `r min_daily_error` cm underestimate and `r max_daily_error` cm 
overestimate of mean daily water level.  

*Align column edges and shaded vertical bar in panel B*
...
Errors at the sub-daily range were present between the raw and corrected. The
two days shown in Figure \@ref(fig:case-study-panel)C have daily mean water
level errors within the expected instrument error. The inflow rate and estimates
of ET derived from these diurnal fluctuations are qualitatively different upon
visual inspection. The uncorrected data shows higher inflow and corresponding
higher ET for periods with similar raw and corrected water level change. To
quantify the qualitative differences E~Sy~ was derived separately using the raw
and corrected data. The respective E~Sy~ functions were then used to separate
the raw and corrected diurnal fluctuations into inflow and ET. During the 
drawdown period used to calculate E~Sy~ the temperature-induced error was generally
small resulting in similar E~Sy~ functions. While there is no
validation data to compare estimated inflow to actual inflow, we can compare
estimated ET to PET. For the following results only dry days were used, where
dry is defined as no daily rainfall or snowmelt greater than 1 mm in a two day
period. ET~raw~ showed negligible correlation with PET (`r pet_cor["raw_et_cm"]`), 
while ET~corr~ showed moderate correlation with PET (`r pet_cor["corrected_et_cm"]`).
Fitting an OLS linear regression between estimated ET and PET showed no 
significant slope between ET~raw~ and PET, but a significant slope, close to
1 for ET~corr~ and PET (Figure \@ref(fig:et-to-pet-panel)). 
...
For both analyses performed in the case study we found significant errors
introduced due to temperature-induced errors in WL~biased~. The purpose of water
level records in monitoring and research is enable a downstream analysis.
Unaccounted for errors in the initial recording have measurable impacts, which
...
errors of 170% and 166% of ET~corr~. In Panel B we see that the
daily mean error on these days show error within the range of expected
instrument error. Quantifiable errors are more likely to be detected (**& to be
of a larger percentage magnitude?**) at finer scales (sub-daily) than at coarser
scales (daily). Figure \@ref(fig:error-scale) shows the result of increasing
data smoothing to identify a scale where temperature errors are not detectable.
It shows that a moving average with a window size of *ZZZ* days is necessary to
completely mask the effects so that 95% of errors are within the range of
instrument error.  
...
likely be large, but slightly smaller than fall errors with low air temperatures
and low water temperatures.

<!-- 
Data doesn't support this straightforward an interpretation but it could be worth exploring this idea more:
During the summer daily mean
error appears to be correlated with response to precipitation events (coincident
peaks in Figure \@ref(fig:case-study-panel) Panels A & B), while the
precipitation response peak in mid-June does not show an increase in daily mean
errors. We suggest that in mid-June the high water levels in the wetland masked
a water level temperature response, while in the drier summer months, wetland
water temperatures showed an increased response, resulting increased error.
-->

It is important to note that while ET~corr~ showed a moderate correlation with
PET a significant slope near 1, the R^2^ of the OLS model was low. This is in
line with other White-method separation studies where low R^2^ values are
observed. @soylu-2012 found R^2^ values of less than 0.2 for the White method
and 0.33--0.4 for their proposed sine-wave based method. @mclaughlin-2014 does
not report R^2^, but their $ET/PET$ index shows high variability, suggesting low
R^2^. Even with direct eddy flux measurement of ET, @lafleur-2005 found an R^2^
as low as 0.56 between ET and PET for a single year of data. Additionally our
wetland could be expected to have poor performance or even be unusable for this
analysis. Nearby surface water dynamics and connectivity to other surface water
sources has been shown to have high levels of interference with determining
E~Sy~ or White-method ET [@watras-2017; @zhu-2011].

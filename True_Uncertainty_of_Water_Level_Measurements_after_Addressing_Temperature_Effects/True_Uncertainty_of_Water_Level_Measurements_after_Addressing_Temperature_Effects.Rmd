...
predictions <- 
  fread(here::here("output/tabular/bootstrap_prediction_values.csv"))
uncertainty <- 
  fread(here::here("output/tabular/bootstrap_predictions_uncertainty_test.csv"))
```
...
AAA). The out of phase variation in air temperature and water
temperature result in the stronger gradient masking the effect of the weaker
gradient. The sequential correction shown in Figure AAA
...
*Figure AAA Three panels showing error ~ air temperature, error ~ water temperature,
and air-temperature-corrected error ~ water temperature*
...
calculated $\hat\epsilon_{L_i}$.

*Figure BBB: A line graph showing the predictions from about ~25 separate bootstrap
models along with the final median prediction and uncertainty derived from all
1000 models*
...
_var-sim_ period. When the two water pressure transducers that showed outlier
model performance (2025928 & 2030899, Figure or Table CCC (RMSE or Sigma plot))
were removed, the increase in RMSE in the _var-sim_ period was reduced to -0.018
and was no longer significant.

_Figure (or table) CCC: Plot or table showing model fit metrics for all 36 final
models to highlight a couple of outliers_
...
training_rmse <- 
  uncertainty[experiment == "var-dis",  .(training_rmse_cm = sqrt(mean(((raw_water_level_cm - predicted_error_cm) - water_depth_cm)^2))), by = .(water_sn, baro_sn)]
...
```{r uncertainty}
uncertainty_tests <- 
  uncertainty[, .(propagated_error_cm = mean(upper_bound_cm - lower_bound_cm),
...
sig_letters <- 
  aov(propagated_error_cm ~ experiment, 
      data = uncertainty[, .(propagated_error_cm = mean(upper_bound_cm - lower_bound_cm),
                             instrument_error_full = mean(2 * qnorm(0.975) * instrument_error_cm)),
...
Instrument accuracy at a 95% confidence band for all models was either `r paste(unique(2*qnorm(0.975)*combined_data$testing$instrument_error_cm), collapse = ", or ")`cm,
...
temperature coefficients. Figure DDD shows that the air temperature coefficients
were grouped by barometric transducer (two distinct regions of overlapping
ranges), and the water temperature coefficients were grouped by water transducer
(18 distinct regions of overlap of the two models representing each water
transducer). Clear transducer differentiation also supports that for absolute
pressure transducers, error is introduced primarily through independent water
and air temperature errors not temperature difference. Correcting for water
level error using temperature difference would disregard the differential
transducer responses. Based on the scale of the coefficients for air temperature
and water temperature, these errors would likely be small in this study. But the
scale of the error would be directly proportional to the sensitivity of
transducer to temperature and would vary between manufacturers and instruments.

_Figure DDD: Plots showing the distribution of bootstrap model coefficients_
...
the beginning and end of the *var-dis* period (Figure EEE). This is further
...
_Figure EEE: A panel of two plots. The first showing time series of corrected
water level to highlight where the error occurs. The second would be a scatter
plot showing residual error as a function of temperature_

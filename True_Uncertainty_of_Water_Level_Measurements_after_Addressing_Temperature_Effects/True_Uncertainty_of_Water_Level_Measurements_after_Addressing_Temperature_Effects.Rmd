...
@mclaughlin-2011 found that temperature differentials between levellogger
...
and streamflow. Correcting for temperature-induced errors is essential to ensure
that the observed flucuations are real and not an artifact. @Cuevas-2010 and
@gribovszki-2013 do validate their measurements with manual readings over the
course of at least one 24-hour period. However, there are no studies to date
examining the combined impact of instrument error and correction error to
estimate the full uncertaintly of measurements. @tamari-2010 highlighted
important differences in how logger uncertainty can be expressed, and how just
instrument error may be enough to mask the targetted signal. Therefore
instrument error, and any uncertainty in correcting the intra- and interlogger
artifacts, should be propagated through to the final water level record.
Propagated uncertainty will provide a detection limit for true diurnal signals.
Without additional validation (through manual measurements or other
instrumentation), diurnal patterns below this detection limit must be considered
artifacts. As such we suggest that estimates of diurnal signal should be a
left-censored dataset.
...
ongoing hydrologic study. Finally we demonstrate the true uncertainty and
compare it to instrument error.
...
variable temperatures and similar and dissimilar air and water thermal regimes.
Eightteen levelloggers of varying age and model, all manufactured by Solinst,
Ltd. (Georgetown, ON, Canada), were used to recorded water levels ( __TABLE
1__). Two levelloggers, both Solinst Levellogger Juniors, were used to monitor
barometric pressure. All levelloggers attempt to provide internal temperature
compenstation on temperature measurments. Reference barometric pressure was
obtained from the Automatic Position Reporting System WX NET/Citizen Weather
Observer Program station FW3311. Station FW3311 is located 3 km away and 77m
higher than the study location. Data for the study period were retreived using
the Synoptic Mesonet API (https://developers.synopticdata.com/mesonet/v2/).
...
## Temperature Correction for Individual Sensors
...
barometric error ($P_{A} - P_{F3311}$) and air temperature. While the loggers
...
As anticipated we observed a trend between temperature-corrected barometric 
error and rate of temperature change ($^oC\;min^{-1}$). A second OLS regression
...
shifts also result in an offset in recorded pressure. This shift can be removed
by applying the cumulative impact of the $\Delta T$ model. These two 
corrections were then applied to the withheld test set (*test-dat*) of data to
evaluate the accuracy of the correction.
...
Unlike air pressure, water temperature showed a correlation between error and
temperature difference.

Water pressure errors were aligned to *stat-sim* because *stat-sim* had the most
stable water temperatures, so trends were induced by temperature difference.

Water temperatures showed no changes in temperature that exceeded one half of 
the resolution of the loggers.


Temperature compensation (below)
was performed using the error of recorded vs reference barometric pressure and
for

Density-compensated pressures were then compared to the , 
altimeter, and sea level pressures. An approximately linear relationship was 
observed between density-compensated pressure and altimeter pressure, with 
deviations seemingly related to air temperature differences due to to the difference in
location as well as siting and radiation shielding temperature differences were
observed between the recorded and Mesowest readings. Data from the *var-dis*
experiment was used The relationship was 
strongest in the *var-dis* experiments, which had the smallest
temperature differences. Developing an logger temperature correction required 
three steps. An ordinarly least squares regression was fit for each logger using 
all available training data. The standardized residuals of the models and 
temperature differences were pooled into a single changepoint regression to 
identify the magnitude of temperature difference before the relationship was 
affected. Data points where the temperature difference was greater than the 
identified threshold were removed and a final OLS model was fit to each logger.


### Temperature Correction for Water Sensors

Although care was taking to provide similar thermal regimes in two of the 
experiments, there were still temperature differences between the water and air
levelloggers. In order to maximize the available data all training data were 
used to develop unrestricted correction models. Change point regression was 
applied on a model comparing the residuals to the temperature difference 
between the water and air sensors. 
Pooled models (both barometric models) showed divergent standardized residuals 
with increasing water-air temperature difference magnitude. This required fitting
two models for each water level logger to determine the temperature difference
threshold. After high-magnitude temperature difference data points were removed
it was possible to use a pooled mixed model to generate a correction.

### Temperature Differential Correction
...
Should temperature difference be absolute temperature difference or does the 
sign of the difference matter. Perhaps more test data would be enough to show 
the impact.
...
Separate out sensor-specific corrections vs generic corrections
...
missing experimental or monitoring data.

Temperature compensation could be overstated based on the time scale of the study.
Would have to know how quickly Solinst changes temperatures in their calibrations.
[@freeman-2004] talks about this stating that it could be a rate of change issue
and/or the fact that parts of the electronics may change temperatures at 
different times.

It is important to note that these rapid temperature changes were short-lived
and the offset of the $\Delta T$ models may decline over time much as an 
intervention effect in a time series model. We tested this on inter-experiment
data excluded from this analysis and found that ... (move from outside to inside
for stat-dis and the long gradual decrease in temperature after a short rapid 
change)
